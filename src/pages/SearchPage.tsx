...
const SearchPage: React.FC = () => {
  ...
  useEffect(() => {
    fetch("https://www.thecocktaildb.com/api/json/v1/1/list.php?c=list")
      .then(res => res.json())
      .then(data => setCategories(data.drinks?.map((d: any) => d.strCategory) || []));
    fetch("https://www.thecocktaildb.com/api/json/v1/1/list.php?g=list")
      .then(res => res.json())
      .then(data => setGlasses(data.drinks?.map((d: any) => d.strGlass) || []));
    fetch("https://www.thecocktaildb.com/api/json/v1/1/list.php?i=list")
      .then(res => res.json())
      .then(data => setIngredientList(data.drinks?.map((d: any) => d.strIngredient1) || []));
  }, []);
  ...
  // Filtrera på ingrediens
        if (ingredient.trim()) {
          const resIng = await fetch(`https://www.thecocktaildb.com/api/json/v1/1/filter.php?i=${encodeURIComponent(ingredient)}`);
          const dataIng = await resIng.json();
          const ids = new Set((dataIng.drinks || []).map((d: any) => d.idDrink));
  ...
  // Endast filter, ingen fritext
        ...
        if (ingredient) {
          ...
          idSets.push(new Set(dataIng.drinks.map((d: any) => d.idDrink)));
          if (!baseList) baseList = dataIng.drinks;
        }
        if (category) {
          const resCat = await fetch(`https://www.thecocktaildb.com/api/json/v1/1/filter.php?c=${encodeURIComponent(category)}`);
          const dataCat = await resCat.json();
          if (!dataCat.drinks) dataCat.drinks = [];
          idSets.push(new Set(dataCat.drinks.map((d: any) => d.idDrink)));
          if (!baseList) baseList = dataCat.drinks;
        }
        if (glass) {
          const resGlass = await fetch(`https://www.thecocktaildb.com/api/json/v1/1/filter.php?g=${encodeURIComponent(glass)}`);
          const dataGlass = await resGlass.json();
          if (!dataGlass.drinks) dataGlass.drinks = [];
          idSets.push(new Set(dataGlass.drinks.map((d: any) => d.idDrink)));
          if (!baseList) baseList = dataGlass.drinks;
        }
  // Intersektera idSets
        let filtered = (baseList || []).filter((d: any) => idSets.every(set => set.has(d.idDrink)));
  // Hämta drinkdetaljer
        const cocktailResults = await Promise.all(filtered.map(async (d: any) => {
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Sök bland tusentals drinkar och cocktails. Filtrera på ingrediens, kategori och glastyp." />
      </Helmet>
  <div className="page-card">
      <form onSubmit={handleSearch} className="search-form">
        ...
        <select value={glass} onChange={e => setGlass(e.target.value)} className="search-select">
          <option value="">Alla glastyper</option>
          {sortedGlasses.map(g => <option key={g} value={g}>{g}</option>)}
        </select>
        <Button type="submit" disabled={loading || (!query.trim() && !ingredient.trim() && !category && !glass)}>
          Sök
        </Button>
      </form>
  ...
  {error && <p className="error-text">{error}</p>}
      ...
      {totalPages > 1 && (
        <div className="search-pagination">
          <Button onClick={() => setPage(p => p - 1)} disabled={page === 1} className="search-pagination-btn">
            Föregående
          </Button>
          <span className="search-pagination-info">Sida {page} av {totalPages}</span>
          <Button onClick={() => setPage(p => p + 1)} disabled={page === totalPages} className="search-pagination-btn">
            Nästa
          </Button>
        </div>
      )}
