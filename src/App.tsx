...
import { CoconutProvider } from './components/coconut/context/CoconutContext';
...
import RealPalmSVG from './components/coconut/svg/RealPalmSVG';
import CoconutPile from './components/coconut/CoconutPile';
...
const App: React.FC = () => {
  ...
  function handleGlobalClick() {
    ...
    if (clickCount.current >= dropEvery.current) {
      if (typeof window !== 'undefined' && window.dropCoconut) {
        window.dropCoconut();
      }
          ...
          <header className="header">
            <div className="header-row">
              <Link to="/" className="logo-link" aria-label="Gå till startsidan">
                ...
                <span className="emoji" role="img" aria-label="drink">
                  🍹
                </span>
              </Link>
            </div>
            <p className="slogan">
              Upptäck, sök och spara dina favoritdrinkar från{' '}
              <span className="slogan-source">The Cocktail DB</span>
            </p>
          </header>
          <nav className="nav" aria-label="Huvudnavigation">
            <NavButton to="/" aria-label="Gå till startsidan">
              <span className="nav-btn-content">
                Hem{' '}
                <span aria-hidden="true" style={{ fontSize: '1.2em', marginLeft: 2 }}>
                  🏠
                </span>
              </span>
            </NavButton>
